...
func (r *Room) animateDoor(dt float64) {
	...
	transformedLeft := mgl32.TransformNormal(mgl32.Vec3{1.0, 0.0, 0.0}, attachPointRotationMatrix)
	transformedFw := mgl32.TransformNormal(mgl32.Vec3{0.0, 0.0, -1.0}, attachPointRotationMatrix)
	...
	rX := mgl32.RadToDeg(float32(math.Asin(float64(-rotationMatrixOnDoorSystem.At(1, 2)))))
	rY := mgl32.RadToDeg(float32(math.Atan2(float64(rotationMatrixOnDoorSystem.At(0, 2)), float64(rotationMatrixOnDoorSystem.At(2, 2)))))
	rZ := mgl32.RadToDeg(float32(math.Atan2(float64(rotationMatrixOnDoorSystem.At(1, 0)), float64(rotationMatrixOnDoorSystem.At(1, 1)))))
	fmt.Printf("---------------\nRx: %f Ry: %f Rz: %f\n", rX, rY, rZ)
